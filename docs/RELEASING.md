...
+ Added the Brew Potion objective.
    + Completes when a player brews any potion.

### Improvements

#### Board

+ Made completed lines glow on the board GUI.

### Fixes

#### Commands

+ Fixed `/bingo time` accepting negative values.
```
...
| `####` | Feature area                   | `Objectives`, `Board`, `Teams`, `GUI`, `Misc`, … — free-form, `Misc` is the catch-all |
...
- Write entries for players and server owners, not developers: "Added the Brew Potion objective", not
  "Refactored YamlObjectiveLoader". Developer-facing changes go under `Technical Details`.
- With outside contributors, append attribution like SkyHanni does:
  `+ Added X. - Name (https://github.com/Trilleo/TriBingo/pull/123)`
...
6. **Check the result** — the `release` workflow under the repo's *Actions* tab builds the jar and creates the GitHub
   Release. Verify the release page shows the changelog text and has `TriBingo-1.1.0.jar` attached.
